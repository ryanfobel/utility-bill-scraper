...
def get_electricity_consumption(soup):
    def find_kWh(tag):
        return tag.name == u'span' and (tag.decode().find('kWh Off Peak') >= 0 or
...
def get_billing_date(soup):
    ...
    match = re.search('([A-Z]+)\s+(\d+)\s+(\d+)', format_fields(soup.find_all(find_billing)[0].next_sibling.next_sibling.span.contents)[0])
