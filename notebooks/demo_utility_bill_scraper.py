...
# # Introduction
#
# This notebook demonstrates scraping of data from a series of utility bills (pdfs). Currently, this library supports:
#
...
# Bills are stored in the tests directory (separate subdirectory for each utility)
...
# convert the list of dictionaries into a dictionary of data frames (one for each utility in the dataset)
...
plt.figure(); df_gas['Gas & Water Charges'].plot(); plt.title('Gas & Water Charges'); plt.ylabel('\$')
plt.figure(); df_gas['Gas Consumption'].plot(); plt.title('Gas Consumption'); plt.ylabel('m$^3$')
plt.figure(); df_gas['Water Consumption'].plot(); plt.title('Water Consumption'); plt.ylabel('m$^3$')

df_gas.to_csv('Kitchener Utilities data.csv')

# Natural gas emission factor
# 119.58 lbs CO2/1000 cubic feet of natural gas
# 1.915 kg CO2/1 m^3 natural gas [119.58 lbs * (1 kg / 2.204623 lbs) * (1 ft^3 / (0.0254 * 12)**3 m^3) / 1000]
...
# Energy per m^3: 1,037 Btu / ft^3 * 1055.1 J / 1 Btu * 1 ft^3 / (0.0254 * 12)**3 m^3
...
print('heating energy per m^3 natural gas (%d%% efficient furnace): %.2f kWh / m^3' % (
...
print('annual CO2 emissions from natural gas: %.1f kg' % (df_gas['Gas Consumption'].sum() * kgCO2_per_cubic_meter))
print('annual cost for natural gas: $%.2f' % (df_gas['Gas Consumption'].sum() * gas_variable_rate))
...
plt.ylim((0, None)); plt.title('Monthly Electricity Consumption'); plt.ylabel('kWh');
...
print('annual electricity usage: %.1f kWh' % (df_electricity['Total Consumption'].sum()))
print('annual electricity cost: $%.2f' % (df_electricity['Amount Due'].sum()))
print('annual CO2 emissions from electricity: %.2f kg' % (
    df_electricity['Total Consumption'].sum() * cabron_intensity_kgCO2_per_kwh))
...
#
# Calculate average monthly usage and costs for offsetting with [Bullfrog Power](https://www.bullfrogpower.com).

# +
print('monthly electricity usage: %.1f kWh (avg), %.1f kWh (max)' % (df_electricity['Total Consumption'].mean(),
                                                                     df_electricity['Total Consumption'].max()))

print('monthly gas usage: %.1f m^3 (avg), %.1f m^3 (max)' % (df_gas['Gas Consumption'].mean(),
                                                             df_gas['Gas Consumption'].max()))

print('\nmonthly electricity offset: $%.2f' % (df_electricity['Total Consumption'].mean() * 0.025))
print('monthly gas offset: $%.2f' % (df_gas['Gas Consumption'].mean() * 0.15))
print('total monthly offset: $%.2f' % (df_electricity['Total Consumption'].mean() * 0.025 + 
