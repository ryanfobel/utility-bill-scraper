...
print(
    "annual CO2 emissions from natural gas: %.1f kg"
    % (df_gas["Gas Consumption"].iloc[-12:].sum() * kgCO2_per_cubic_meter)
)
print(
    "annual cost for natural gas: $%.2f (Fixed: $%.2f, Variable $%.2f)"
    % (
...
# +
print(
    "monthly electricity usage: %.1f kWh (avg), %.1f kWh (max)"
    % (
...
print(
    "monthly gas usage: %.1f m^3 (avg), %.1f m^3 (max)"
    % (
        df_gas["Gas Consumption"].iloc[-12].mean(),
        df_gas["Gas Consumption"].iloc[-12].max(),
    )
)
